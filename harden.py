...
def get_jailbroken_suffix_instructions():
    ...
    train, test = dataset.train_test_split(test_size=0.2, seed=42, shuffle=True).values()
    return train, test
...
refusal_instructions_train = [x['Goal'] for x in refusal_instruction_train]
refusal_instructions_test = [x['Goal'] for x in refusal_instruction_test]
...
for layer_num in range(1,model.cfg.n_layers):
    ...
    for layer in activation_layers:
        ...
        jailbreak_direction = jailbreak_direction / (jailbreak_direction.norm() + EPSILON)
...
'''activation_jailbreaks = torch.load('jailbreak_dirs.pth')
jailbreak_dirs = activation_jailbreaks'''
...
We can implement the intervention equivalently by directly orthogonalizing the weight matrices that write to the residual stream with respect to the jailbreak direction $\widehat{r}$:
